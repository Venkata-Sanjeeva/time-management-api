...
public class EmployeeController_V1 {
	...
	@Autowired
	private EmployeeServiceImpl_V1 empService;
	
	@PostMapping("/create")
	public ResponseEntity<GlobalResponse<EmployeeResponseV1>> createEmployee(@RequestBody EmployeeRequest empReq, Principal principal) {
		return ResponseEntity.status(HttpStatus.CREATED)
				.body(GlobalResponse.<EmployeeResponseV1>builder()
	...
	@GetMapping("/read/{id}")
	public ResponseEntity<GlobalResponse<EmployeeResponseV1>> readEmployee(@PathVariable(name = "id") String empId) {
	...
	@GetMapping("/read/all")
	public ResponseEntity<GlobalResponse<List<EmployeeResponseV1>>> readAllEmployeesRelatesToUser(Principal principal) {
		return ResponseEntity.status(HttpStatus.OK)
				.body(GlobalResponse.<List<EmployeeResponseV1>>builder()
				.status(HttpStatus.OK.value())
	...
	public ResponseEntity<GlobalResponse<EmployeeResponseV1>> updateEmployee(
